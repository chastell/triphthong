...
module Triphthong describe Verse do
  describe '#has_caesura_after?' do
    it 'returns a Boolean on whether the given String has a cæsura after the given syllable' do
      refute Verse.new('Litwo! Ojczyzno moja! ty jesteś jak zdrowie!').has_caesura_after? 6
      assert Verse.new('Litwo! Ojczyzno moja! ty jesteś jak zdrowie!').has_caesura_after? 7
      refute Verse.new('Litwo! Ojczyzno moja! ty jesteś jak zdrowie!').has_caesura_after? 9
      refute Verse.new('Litwo! Ojczyzno moja! ty jesteś jak zdrowie!').has_caesura_after? 12
      assert Verse.new('Litwo! Ojczyzno moja! ty jesteś jak zdrowie!').has_caesura_after? 13
      refute Verse.new('Litwo! Ojczyzno moja! ty jesteś jak zdrowie!').has_caesura_after? 14
  ...
  describe '#rhyme_pattern' do
    ...
    it 'works with Pan Tadeusz rhymes' do
      ...
      File.read('spec/fixtures/pan-tadeusz.txt').lines.map(&:chomp).reject(&:empty?).each_slice 2 do |a, b|
        Verse.new(a).rhyme_pattern.must_equal Verse.new(b).rhyme_pattern, "rhyme mismatch between ‘#{a}’ and ‘#{b}’"
      end
    end
  end
  ...
  describe '#syllable_count' do
    it 'returns the number of syllables' do
      ...
      File.read('spec/fixtures/pan-tadeusz.txt').lines.map(&:chomp).each do |line|
        ...
        Verse.new(line).syllable_count.must_equal 13, "not 13 syllables in ‘#{line}’"
      end
    end
    ...
    it 'handles edge cases' do
      {
        ...
        Verse.new("z #{word} z").syllable_count.must_equal count, "not #{count} syllables in ‘#{word}’"
      end
    end
  end
end end
